...
def main():
    ...
    # 件数が多くなりがちなので、1回のリクエストが大きくなりすぎないようchunkに分けて投入する
    chunk_size = 5000
    failed_chunks = []
    for start in range(0, len(all_rows), chunk_size):
        ...
        try:
            ...
            if res.status_code >= 400:
                failed_chunks.append((start, start + len(chunk)))
        except Exception as e:
            print(f'{start}〜{start + len(chunk)}件目: 例外が発生しました({e})。このチャンクはスキップして続行します。')
            failed_chunks.append((start, start + len(chunk)))

    if failed_chunks:
        print(f'\n警告: 投入に失敗したチャンクが {len(failed_chunks)} 件あります: {failed_chunks}')
