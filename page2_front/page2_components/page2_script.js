...
// 生成履歴を読み込む関数（site_idでグループ化された、派生数付きの一覧を表示）
async function loadPageHistory() {
    ...
    try {
        ...
        listEl.innerHTML = data.pages.map(page => {
            ...
            const derivedText = page.derived_count > 0 ? `（${page.derived_count}個派生）` : "";
            return `
                <li>
                    <a href="/api/page2/render/${page.id}" target="_blank">${page.prompt}${derivedText}</a>
